...
GIT_IMPORT=github.com/devopracy/devopracy-cli/version
...
dev: deps ## Build and install a development build
	...
	@cp $(GOPATH)/bin/devopracy-cli bin
	@cp $(GOPATH)/bin/devopracy-cli pkg/$(GOOS)_$(GOARCH)
...
testacc: deps generate ## Run acceptance tests
	...
	devopracy-cli_ACC=1 go test -v $(TEST) $(TESTARGS) -timeout=45m

testrace: deps ## Test for race conditions
	@go test -race $(TEST) $(TESTARGS) -timeout=2m

updatedeps:
	@echo "INFO: devopracy-cli deps are managed by govendor. See CONTRIBUTING.md"
